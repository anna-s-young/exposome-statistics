...
data_quantized = data # use data file where each chemical is in a separate column (observations in rows)
...
pairs = unique(data_quantized$PairSet) # use variable that indicates the ID of the matched pair
temp = data_quantized
for (r in 1:n_RHs) { # use the number of repeated holdouts
  pairs_partition = sample(c(F,T), size=length(pairs), replace=T, prob=c(1-0.6, 0.6)) # randomly shuffle by pair (60% validation set)
...
ggplot(holdouts_stats %>% filter(mix_name %in% chemicals_to_display), aes(y=mix_name, x=percent_rh_above_threshold)) + 
  ...
  scale_x_continuous(name="% of Holdouts with Weight > Threshold (Barchart)", limits=c(0, max(holdouts_stats$percent_rh_above_threshold)),
